...
import com.alibaba.nacos.plugin.auth.impl.constant.DatabaseTypeConstant;
...
/**
 * Oracle page handler adapter.
 *
 * @author huangKeMing
 */
public class OralcePageHandlerAdapter implements PageHandlerAdapter {
    
    @Override
    public boolean supports(String dataSourceType) {
        return DatabaseTypeConstant.ORACLE.equals(dataSourceType) || DatabaseTypeConstant.DM.equals(dataSourceType);
    }
    
    @Override
    public OffsetFetchResult addOffsetAndFetchNext(String fetchSql, Object[] arg, int pageNo, int pageSize) {
        if (!fetchSql.contains(AuthPageConstant.ROWNUM)) {
            fetchSql += " " + " ROWNUM <= " + AuthPageConstant.FIRST_MARK + " ) WHERE ROW_ID > " + AuthPageConstant.SECOND_MARK;
