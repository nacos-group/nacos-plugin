...
/**
 * YasdbDatabaseDialectTest.
 *
 * @author Nacos
 */
public class YasdbDatabaseDialectTest {
    
    @Test
    public void testZeroConfigContract() {
        DatabaseDialect dialect = new YasdbDatabaseDialect();
        
        PluginConfigSpec configSpec = (PluginConfigSpec) dialect;
        Assert.assertEquals(DatabaseTypeConstant.YASDB, dialect.getType());
    ...
    public void testDiscoverableThroughSpi() {
        Assert.assertTrue(isDiscoverable(DatabaseTypeConstant.YASDB));
    }
    
    private boolean isDiscoverable(String type) {
        for (DatabaseDialect dialect : ServiceLoader.load(DatabaseDialect.class)) {
            if (type.equals(dialect.getType()) && dialect instanceof YasdbDatabaseDialect) {
                return true;
            }
        }
        return false;
