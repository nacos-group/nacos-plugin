 ...
 * @author Ken
 */
public class ConfigMigrateMapperByOracle extends AbstractOracleMapper implements ConfigMigrateMapper {
    
    @Override
    public MapperResult findConfigIdNeedInsertMigrate(MapperContext context) {
        String sql = getLimitTopSqlWithMark("SELECT ci.id FROM config_info ci WHERE ci.tenant_id = '' AND NOT EXISTS "
                + " ( SELECT 1 FROM config_info ci2  WHERE ci2.data_id = ci.data_id AND ci2.group_id = ci.group_id AND ci2.tenant_id = 'public' )"
                + " AND ci.id > ?" + " ORDER BY ci.id ");
    ...
    public MapperResult findConfigNeedUpdateMigrate(MapperContext context) {
        String sql = getLimitTopSqlWithMark("SELECT ci.id, ci.data_id, ci.group_id, ci.tenant_id"
                ...
                + " AND ci2.gmt_modified < ci.gmt_modified )" + " AND id > ?" + " ORDER BY id ");
        return new MapperResult(sql, CollectionUtils.list(context.getWhereParameter(FieldConstant.SRC_TENANT),
    ...
    public MapperResult findConfigGrayNeedUpdateMigrate(MapperContext context) {
        String sql = getLimitTopSqlWithMark("SELECT ci.id, ci.data_id, ci.group_id, ci.tenant_id, ci.gray_name "
                ...
                + " AND ci2.gmt_modified < ci.gmt_modified )" + " AND ci.id > ?" + " ORDER BY ci.id ");
        return new MapperResult(sql, CollectionUtils.list(context.getWhereParameter(FieldConstant.SRC_TENANT),
                context.getWhereParameter(FieldConstant.SRC_USER),
                context.getWhereParameter(FieldConstant.TARGET_TENANT),
                context.getWhereParameter(FieldConstant.SRC_USER), context.getWhereParameter(FieldConstant.ID),
    ...
    public MapperResult findConfigGrayIdNeedInsertMigrate(MapperContext context) {
        String sql = getLimitTopSqlWithMark(
                "SELECT ci.id FROM config_info_gray ci WHERE ci.tenant_id = '' AND NOT EXISTS "
                        + " ( SELECT 1 FROM config_info_gray ci2  WHERE ci2.data_id = ci.data_id AND ci2.group_id = ci.group_id"
