...
/**
 * DaMengDatabaseDialectTest.
 *
 * @author Nacos
 */
public class DaMengDatabaseDialectTest {

    @Test
    public void testZeroConfigContract() {
        DatabaseDialect dialect = new DaMengDatabaseDialect();

        PluginConfigSpec configSpec = (PluginConfigSpec) dialect;
        Assert.assertEquals(DatabaseTypeConstant.DM, dialect.getType());
    ...
    public void testDiscoverableThroughSpi() {
        Assert.assertTrue(isDiscoverable(DatabaseTypeConstant.DM));
    }
    ...
    private boolean isDiscoverable(String type) {
        for (DatabaseDialect dialect : ServiceLoader.load(DatabaseDialect.class)) {
            if (type.equals(dialect.getType()) && dialect instanceof DaMengDatabaseDialect) {
                return true;
            }
        }
        return false;
