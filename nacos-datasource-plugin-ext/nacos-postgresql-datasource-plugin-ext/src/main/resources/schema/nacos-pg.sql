...
(
    "id"                 BIGINT        NOT NULL, -- 对应原表的 config_info ID
    "nid"                BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    "data_id"            VARCHAR(255)  NOT NULL,
    "group_id"           VARCHAR(128)  NOT NULL,
    "app_name"           VARCHAR(128)           DEFAULT NULL,
...
COMMENT ON TABLE "his_config_info" IS '多租户改造 - 配置历史版本表';
...
COMMENT ON COLUMN "tenant_info"."tenant_id" IS 'Tenant ID';
COMMENT ON COLUMN "tenant_info"."tenant_name" IS '租户名称';
COMMENT ON COLUMN "tenant_info"."tenant_desc" IS '租户描述';
...
COMMENT ON TABLE "users" IS 'Nacos 用户表';
...
COMMENT ON TABLE "roles" IS 'Nacos 角色映射表';
...
COMMENT ON TABLE "permissions" IS 'Nacos 权限明细表';
