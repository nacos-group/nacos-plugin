...
/**
 * GaussdbDatabaseDialectTest.
 *
 * @author Nacos
 */
public class GaussdbDatabaseDialectTest {

    @Test
    public void testZeroConfigContract() {
        DatabaseDialect dialect = new GaussdbDatabaseDialect();

        PluginConfigSpec configSpec = (PluginConfigSpec) dialect;
        Assert.assertEquals(DatabaseTypeConstant.GUASSDB, dialect.getType());
    ...
    public void testDiscoverableThroughSpi() {
        Assert.assertTrue(isDiscoverable(DatabaseTypeConstant.GUASSDB));
    }
    ...
    private boolean isDiscoverable(String type) {
        for (DatabaseDialect dialect : ServiceLoader.load(DatabaseDialect.class)) {
            if (type.equals(dialect.getType()) && dialect instanceof GaussdbDatabaseDialect) {
                return true;
            }
        }
        return false;
