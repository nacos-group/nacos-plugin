...
public class SqlServerDatabaseDialectTest {
    ...
    public void testZeroConfigContract() {
        DatabaseDialect dialect = new SqlServerDatabaseDialect();
        
        PluginConfigSpec configSpec = (PluginConfigSpec) dialect;
        Assert.assertEquals(DatabaseTypeConstant.SQLSERVER, dialect.getType());
    ...
    public void testDiscoverableThroughSpi() {
        Assert.assertTrue(isDiscoverable(DatabaseTypeConstant.SQLSERVER));
    }
    
    private boolean isDiscoverable(String type) {
        for (DatabaseDialect dialect : ServiceLoader.load(DatabaseDialect.class)) {
            if (type.equals(dialect.getType()) && dialect instanceof SqlServerDatabaseDialect) {
                return true;
            }
        }
        return false;
