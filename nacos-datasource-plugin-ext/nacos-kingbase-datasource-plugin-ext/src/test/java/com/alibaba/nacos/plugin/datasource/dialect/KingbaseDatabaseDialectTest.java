...
/**
 * KingbaseDatabaseDialectTest.
 *
 * @author Nacos
 */
public class KingbaseDatabaseDialectTest {

    @Test
    public void testZeroConfigContract() {
        DatabaseDialect dialect = new KingbaseDatabaseDialect();

        PluginConfigSpec configSpec = (PluginConfigSpec) dialect;
        Assert.assertEquals(DatabaseTypeConstant.KINGBASE, dialect.getType());
    ...
    public void testDiscoverableThroughSpi() {
        Assert.assertTrue(isDiscoverable(DatabaseTypeConstant.KINGBASE));
    }
    ...
    private boolean isDiscoverable(String type) {
        for (DatabaseDialect dialect : ServiceLoader.load(DatabaseDialect.class)) {
            if (type.equals(dialect.getType()) && dialect instanceof KingbaseDatabaseDialect) {
                return true;
            }
        }
        return false;
