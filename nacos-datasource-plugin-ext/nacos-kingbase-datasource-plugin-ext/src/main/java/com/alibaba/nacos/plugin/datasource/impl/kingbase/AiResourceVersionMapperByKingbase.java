 ...
 * @author Ken
 */
public class AiResourceVersionMapperByKingbase extends AbstractMapperByKingbase implements AiResourceVersionMapper {
    
    @Override
    public MapperResult findAiResourceVersionFetchRows(MapperContext context) {
        WhereBuilder where = new WhereBuilder(
                "SELECT id,gmt_create,gmt_modified,type,author,name,c_desc,status,version,namespace_id,storage,"
                        + "publish_pipeline_info,download_count FROM ai_resource_version");
        ...
        String sql = getLimitPageSqlWithOffset(built.getSql() + " ORDER BY gmt_modified DESC ", context.getStartRow(),
                context.getPageSize());
