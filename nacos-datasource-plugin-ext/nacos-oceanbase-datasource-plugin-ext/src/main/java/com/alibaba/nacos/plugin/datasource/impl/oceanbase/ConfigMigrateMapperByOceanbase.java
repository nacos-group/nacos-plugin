 ...
 * @author Ken
 */
public class ConfigMigrateMapperByOceanbase extends AbstractOceanbaseMapper implements ConfigMigrateMapper {
    ...
    public MapperResult findConfigGrayIdNeedInsertMigrate(MapperContext context) {
        String sql = getLimitTopSqlWithMark("SELECT ci.id FROM config_info_gray ci WHERE ci.tenant_id = '' AND NOT EXISTS "
                + " ( SELECT 1 FROM config_info_gray ci2  WHERE ci2.data_id = ci.data_id AND ci2.group_id = ci.group_id"
